...
export const TaskBoardPage: React.FC = () => {
      ...
      if (previousTasks) {
        queryClient.setQueryData<Task[]>(['tasks'], 
          previousTasks.map(t => t.taskId === taskId ? { 
            ...t, 
            status, 
            completionPercent: status === 'COMPLETED' ? 100 : status === 'NOT_STARTED' ? 0 : t.completionPercent 
          } : t)
        );
      }
  ...
  const handleDrop = (e: React.DragEvent, targetStatus: string) => {
    ...
    // Check status transition permission (Leads/President can move any; Members can only move their own assigned tasks)
    const isAssignedToMe = task.assignedTo === myUserId;
    const isMyTeamLead = role === 'LEAD' && task.teamId === myTeamId;
    const isExecutive = role === 'PRESIDENT' || role === 'VP';
  ...
  const calculateMetrics = () => {
    ...
    if (total === 0) return { total: 0, completed: 0, completionRate: 0, delayed: 0, blocked: 0, overdue: 0 };
    ...
    return { total, completed, completionRate, delayed, blocked, overdue };
  };
    ...
    <div className="space-y-lg animate-fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-md">
        <div className="space-y-xxs">
          ...
          <p className="text-caption-spec text-ink-muted48">Track workloads, update progress milestones, and manage task assignments.</p>
        </div>
        
        {/* Create Task Action (President, VP, Leads can create tasks) */}
        {(role === 'PRESIDENT' || role === 'VP' || role === 'LEAD') && (
          <button 
          ...
          return (
            ...
            >
              ...
              <div className={`px-sm py-xxs border rounded-pill flex items-center justify-between font-text text-[12px] font-semibold mb-sm ${lane.color}`}>
                ...
                <span className="w-5 h-5 rounded-full bg-canvas border border-hairline flex items-center justify-center text-[10px] text-ink">
                  {laneTasks.length}
                </span>
              </div>
                  ...
                  laneTasks.map(task => {
                    ...
                    return (
                      <div
                        ...
                        className={`bg-canvas border border-hairline rounded-md p-md shadow-product-surface space-y-xs cursor-pointer hover:border-primary hover:shadow-md transition-all active:scale-[0.98] text-left`}
                      >
...
const EditTaskModal: React.FC<EditModalProps> = ({ task, teams, users, events, canEditAdminFields, onSubmit, onClose }) => {
  ...
  // Safe checks for member updates (COMPLETED forces 100%, NOT_STARTED forces 0%)
      ...
      <div className="bg-canvas border border-hairline sm:rounded-lg rounded-t-xl shadow-product-surface max-w-[500px] w-full p-lg space-y-md animate-scale-up text-left max-h-[92dvh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-hairline pb-xs">
          <div className="space-y-[2px]">
            ...
            <h3 className="text-body-strong font-bold text-ink">Modify Task Status</h3>
          </div>
          <button onClick={onClose} className="p-xxs hover:bg-ink-muted8 rounded-md transition">
            <X className="w-4 h-4 text-ink-muted48 hover:text-ink" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-sm text-[13px]">
          ...
          {/* Section 1: Standard Progress updates (Always editable by operators/assignees) */}
          ...
          {/* Section 2: Administrative Details (Read Only for general members, editable by Lead/President) */}
