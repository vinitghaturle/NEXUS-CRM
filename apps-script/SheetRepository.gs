...
SheetRepository.prototype = {
  ...
  _getData: function() {
    ...
    if (lastRow <= 1) {
      var defaultHeaders = this._getSchemaHeaders();
  ...
  getAll: function() {
    ...
    return data.rows.filter(function(row) {
      ...
      // 04_Tasks and 12_Issues soft-delete: status === CANCELLED / "CANCELLED"
      if (row.hasOwnProperty("status")) {
        if (row.status === "CANCELLED" || row.status === "CANCELLED") {
          // Note: for tasks, cancelled status means soft deleted
          if (self.sheetName === "04_Tasks") {
            return false;
          }
        }
      }
      ...
      if (row.hasOwnProperty("eventStatus")) {
        if (row.eventStatus === "CANCELLED" || row.eventStatus === "CANCELLED") {
          return false;
        }
      }
      // Documents soft-delete: archived === TRUE / "TRUE"
      if (row.hasOwnProperty("archived")) {
        if (row.archived === true || row.archived === "TRUE") {
          return false;
        }
      }
      return true;
